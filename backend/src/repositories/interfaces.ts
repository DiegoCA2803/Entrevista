...
export interface IEquipmentRepository {
  ...
  create(equipment: Omit<Equipment, 'id' | 'created_at' | 'updated_at'> & { id?: string }): Promise<Equipment>;
...
export interface IOperatorRepository {
  ...
  create(operator: Omit<Operator, 'id' | 'created_at' | 'certifications'> & { id?: string }): Promise<Operator>;
...
export interface IShiftRepository {
  ...
  updateStatus(id: string, status: ShiftStatus, actualHours?: number, closedBy?: string, notes?: string): Promise<Shift>;
  ...
  updateAssignmentStatus(id: string, status: Assignment['status'], riskReason?: string | null): Promise<Assignment>;
}
